...
class MonthlyReset(commands.Cog):
    ...
    async def monthly_reset(self, ctx):
        announce_channel_id = 1054539034921271356
        announce = self.bot.get_channel(announce_channel_id)
        ...
        if now.day == 1 and now.hour == 0 and now.minute == 1:
            ...
            top_result = await query(returntype="one", sql="""SELECT member_id, month_lvl, month_exp, coins FROM members 
